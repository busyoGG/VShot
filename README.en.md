...
| **Recording/replaying a window** (`record window`, `replay start window`) | `ext_image_copy_capture_v1` (the window's own dma-buf) | ❌ none (the protocols offer `zwlr_screencopy` for outputs and no toplevel capture at all) | **KWin `ScreenShot2.CaptureWindow`** (captured by the window's `QUuid`, CPU pixels) | ❌ none | ❌ none |
| Following the focus (`--follow`) | ✅ `hyprctl activewindow` | ⚠️ the query exists (`niri msg focused-window`), but window recording itself is unsupported | ✅ scripting probe / `kdotool` | ⚠️ the query exists, but window recording itself is unsupported | ❌ none |
| Window pid for `--app-audio` | `hyprctl clients -j` | ⚠️ `niri msg windows` has one, but window recording itself is unsupported | ✅ the scripting probe's `pid` field | ❌ none | ❌ none |
...
- **Per-application audio** — measured on Hyprland: two mpv players at 880 Hz and 220 Hz produced a `record window --app-audio` file whose dominant frequency is 880 Hz, so the isolation holds; a window playing nothing degrades to a video-only recording. The window pid has a source on every compositor where a window can be recorded at all (Hyprland and niri report it directly, KWin through the scripting probe's `pid` field) — niri's is moot, since niri cannot record a window in the first place, see below — so `--app-audio` works on Plasma too; Sway and labwc have no pid source and refuse plainly instead of quietly falling back to the microphone. The niri and KWin pid paths have unit tests and headless live runs but no isolation measurement against a real audio session.
...
- **niri** — both the tiled and floating **screenshot** paths were verified on a real session: tiled windows were measured with two side-by-side kitty windows (residual 0.45/0.51 per channel), and floating windows go through niri's `tile_pos_in_workspace_view` coordinates plus `matches_at_position` verification, with correct results on the real session. If a floating window capture comes out misaligned, `--no-blend` bypasses the locating step. niri's focus query (`niri msg --json focused-window`) and its window list are wired into the shared window table, which feeds the picker and the pixel detector's candidates. **A window cannot be recorded on niri, though**: measured against niri 25.11, its Wayland globals offer `zwlr_screencopy_manager_v1` (output capture) and `zwlr_foreign_toplevel_manager_v1`, and **none** of `ext_foreign_toplevel_list_v1`, `ext_foreign_toplevel_image_capture_source_manager_v1` or `ext_image_copy_capture_manager_v1`, so `record window` and `replay start window` both fail at the connect step with a message saying so. Its portal has no ScreenCast implementation either (`niri-portals.conf` maps only Access, Notification and Secret), so `--portal` does not cover it. The only way to get "one window" out of niri is to record an output and crop — that is a region recording, not a window recording.
