...
A compositor sends no new frame while the window's content does not change. The
replay then sends the frame it is holding once more, at the session's own frame
rate, so the ring's timeline keeps step with the clock: after ten seconds of a
still window, a save still reaches back over the last ten seconds. **A recording
does not do this** — in a recording a still picture is one long frame whose
length the end of the loop supplies, so there is nothing to repeat and no
encoding cost while nothing moves.
