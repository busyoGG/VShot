...
窗口内容不变时，合成器不会送新帧。回录在这时按会话的帧率把手里那一帧再送一次，让环的时间线
始终跟着真实时间走——静止十秒后按保存，拿到的仍是最后十秒。**录制不这么做**：一条录制里的静止
画面就是一帧长帧，长度由收尾补上，不必重复编码，静止时也就没有编码开销。
